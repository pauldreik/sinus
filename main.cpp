...
double
runBenchmark(std::size_t Nelements, DataKind kind, int seed)
{
  ...
  const auto nof_iterations =
    static_cast<int>(1.0 * 3e8 / Nelements * 8 / sizeof(T));
  ...
  std::cout << "the sum is " << sum << " speed is " << Nvals << " in " << dt
...
struct Std_sin : ImplementationBase
{
  ...
  // garbage - 100 cycles/element (because of how double is represented, this is
  // mostly huge numbers)
